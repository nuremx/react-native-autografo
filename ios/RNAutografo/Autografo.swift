...
//
import UIKit class AutografoView : UIView {
    private let signatureViewController = SignatureDrawingViewController ()

    override init (frame: CGRect) {
        super.init (frame: frame)

        self.backgroundColor = UIColor.white self.addSubview (signatureViewController.view)

        signatureViewController.view.clipsToBounds = true self.addConstraints (
            [
                NSLayoutConstraint.init (
                    item : signatureViewController.view,
                    attribute : .leading,
                    relatedBy : .equal,
                    toItem : self,
                    attribute : .leading,
    ...
    @objc
    var onSave : RCTDirectEventBlock? @objc var strokeColor : NSString? {
        didSet {
    ...
    func sendBase64String () {
        ...
        let string = pngData?.base64EncodedString (options: .lineLength64Characters) ?? "" onSave! (["base64": string as NSString])
    }

    func saveImageAsFile (_ fileName: NSString?) {
        ...
        if let filePath = paths.first?.appendingPathComponent (fileName != nil ? "\(fileName!).png" : "signature.png") {
            // Save image.
            do {
